...
sitevars <- filter(sitevars, domain %in% elev_extreme_domains)
```
...
print('all shared variables')
print(unique(sitevars$variable_code))

print('vars recorded for >= 5 years, at >= average rate of 12 obs/year')
sitevars %>% 
    mutate(ndays = difftime(last_record_utc, first_record_utc, unit = 'days'),
           nyears = as.numeric(ndays / 365.25)) %>% 
    filter(domain %in% elev_extreme_domains,
           nyears >= 5,
           )
    filter(variable_code %in% 
