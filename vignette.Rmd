...
elev_extreme_domains <- ws_smry %>% 
    ...
    slice(c(1:2, (n() - 1):n())) %>% #2 highest and 3 lowest domains, by average watershed elevation
    print() %>%
    pull(domain)
```
...
dmn_colors <- factor(str_split_fixed(colnames(doc)[-1], '__', n = 2)[, 1])
levels(dmn_colors) <- hcl.colors(length(elev_extreme_domains))

dg <- dygraph(xts(x = doc[, -1], order.by = doc$datetime)) %>% 
    dyRangeSelector()
for(i in 2:ncol(doc)){
    dg <- dySeries(dg, colnames(doc)[i], color = as.character(dmn_colors[i - 1]))
}
dg
```
...
NO3_N_flux <- ms_calc_flux(NO3_N_conc, Q, q_type = 'discharge')

#after
filter(NO3_N_flux, datetime == as.POSIXct('1985-05-09 00:00:00', tz = 'UTC'), site_code == 'ALBION')
```

### Compute annual nitrate-N load (still in beta)

```{r load calc}

# `:::` is used to call internal package functions. this function will be
# officially included in the macrosheds package when we release version 2 of
# the MacroSheds dataset, which will include monthly and yearly load estimates
# based on Gubbins et al. in prep
NO3_N_load <- macrosheds:::ms_calc_flux_rsfme(NO3_N_conc, Q, 
```
