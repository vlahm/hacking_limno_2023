...
plot_conc <- function(ms_root, chemvar){
    ...
    ylims[ylims == 0] <- 0.001
    ylims <- log(ylims)
    chemvar_pretty <- sub('_', '-', chemvar)
    plot(1:nsites, rep(0, nsites), ylim = ylims, type = 'n', yaxt = 'n',
         ylab = paste0('log ', chemvar_pretty, ' (', unit, ')'),
         xlab = '', xaxt = 'n', yaxs = 'i', cex.lab = 1.3, font.lab = 2,
         main = paste(chemvar_pretty, 'distribution across MacroSheds sites'), xlim = c(1, nsites))
    axis(2, at = log(log_ticks), labels = log_ticks, cex.axis = 1.3)
    corners = par("usr")
    ...
    # }
    text(x = quantile(1:nsites, 0.15), font = 2,
         y = quantile(ylims, 0.95), adj = 0, col = 'white',
...
# png(res = 300, height = 10, width = 12, units = 'in',
#     filename = '~/git/macrosheds/hacking_limno_macrosheds/chemdist.png')
